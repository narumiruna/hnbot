...
- HN comment URL and canonical positive numeric ID
...
`Article` contains a title, summary, and ordered `Section` list. Each section contains a title, one emoji, and body content. The OpenAI request includes a strict schema generated by `schemars`; returned content is parsed with `serde_json` and checked against length constraints, including Telegraph's 1–256 character title boundary.
...
The adapter lazily creates one account per process and reuses its access token, sanitizes generated HTML to Telegraph's allowed tags and attributes, converts it into Telegraph Node JSON, and creates pages. Unsupported or mismatched tags are escaped rather than executed.

### Telegram

The adapter posts `chat_id`, escaped HTML text, and `parse_mode=HTML` to `sendMessage`. Titles, summaries, source URLs, discussion URLs, and page URLs are escaped before interpolation, and visible text is Unicode-safely truncated to Telegram's 4096-character limit.

## Concurrency and failure boundaries

Duplicate IDs in one feed batch are collapsed before work starts. Two Tokio semaphores then apply per batch:
...
- idempotent Redis lookups and writes retry up to three times without replaying generation, publishing, or notification side effects
- Telegram failure, or Redis failure after its local retries, makes the batch fail after sibling futures finish
...
Rust tests cover settings, CLI, RSS parsing and ID normalization, Algolia discussion rendering/routing, retry/cooldown, pacing, timeout separation and diagnostics, Redis authentication and local retry behavior, content conversion, Unicode chunking, structured OpenAI requests, Telegraph sanitization and account reuse, bounded Telegram payloads, duplicate-feed dedupe ordering, partial failures, polling cancellation, and a fully mocked end-to-end service flow.
